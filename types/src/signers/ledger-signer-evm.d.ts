...
export default class LedgerSignerEvm implements ISignerEvm {
   ...
   */
  signAuthorization(
    _auth: import("ethers").AuthorizationRequest,
  ): Promise<import("ethers").Authorization>;
  /** Clear device handles and local state. */
  dispose(): void;
}
export type UnsignedEvmTransaction =
  import("../utils/tx-populator-evm.js").UnsignedEvmTransaction;
export type EvmWalletConfig =
  import("../wallet-account-read-only-evm.js").EvmWalletConfig;
export type DeviceManagementKit =
  import("@ledgerhq/device-management-kit").DeviceManagementKit;
